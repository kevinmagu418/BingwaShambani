...
async verifyEmailCode(code: string) {
  ...
  if (!user || user.isVerified) throw new AuthError('Invalid or already used verification code');
  ...
  /*  GitHub / Google OAuth*/
  async findOrCreateOAuth(dto: OAuthRegisterDTO) {
